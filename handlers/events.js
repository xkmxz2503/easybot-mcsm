      ...
      if (!data) {
        ...
        for (const inst of instances) {
          ...
          if (event.RawMessage === "开启 " + serverName && event.PeerId === groupuuid && isOwner && use_regex === false) {
            const result = await httpRequest_on(inst);
            const chain = new MessageChain()
              .Text("已执行开启服务器: ").Text(serverName)
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            ...
            const result = await httpRequest_off(inst);
            const chain = new MessageChain()
              .Text("已执行关闭服务器: ").Text(serverName)
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            return;
            ...
            const result = await httpRequest_restart(inst);
            const chain = new MessageChain()
              .Text("已执行重启服务器: ").Text(serverName)
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            return;
            ...
            const result = await httpRequest_kill(inst);
            const chain = new MessageChain()
              .Text("已执行强制关闭服务器: ").Text(serverName)
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            return;
        ...
        for (const inst of instances) {
          ...
          if (regexOn.test(event.RawMessage) && event.PeerId === groupuuid && isOwner && use_regex === true) {
            const result = await httpRequest_on(inst);
            const chain = new MessageChain()
              .Text("已执行开启服务器: ").Text(serverName).Text(" (正则匹配)")
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            logger.info(`[匹配分支] 开启 | serverName: ${serverName} | 消息: ${event.RawMessage}`);
            return;
          }
          else if (regexOff.test(event.RawMessage) && event.PeerId === groupuuid && isOwner && use_regex === true) {
            const result = await httpRequest_off(inst);
            const chain = new MessageChain()
              .Text("已执行关闭服务器: ").Text(serverName).Text(" (正则匹配)")
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            logger.info(`[匹配分支] 关闭 | serverName: ${serverName} | 消息: ${event.RawMessage}`);
            return;
          }
          else if (regexRestart.test(event.RawMessage) && event.PeerId === groupuuid && isOwner && use_regex === true) {
            const result = await httpRequest_restart(inst);
            const chain = new MessageChain()
              .Text("已执行重启服务器: ").Text(serverName).Text(" (正则匹配)")
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            logger.info(`[匹配分支] 重启 | serverName: ${serverName} | 消息: ${event.RawMessage}`);
            return;
          }
          else if (regexKill.test(event.RawMessage) && event.PeerId === groupuuid && isOwner && use_regex === true) {
            const result = await httpRequest_kill(inst);
            const chain = new MessageChain()
              .Text("已执行强制关闭服务器: ").Text(serverName).Text(" (正则匹配)")
              .NextLine()
              .Text(result.success ? "✅ 操作成功" : "❌ 操作失败：" + result.msg);
            event.Context.Reply(chain);
            logger.info(`[匹配分支] 强制关闭 | serverName: ${serverName} | 消息: ${event.RawMessage}`);
            return;
          }
        }
      }
