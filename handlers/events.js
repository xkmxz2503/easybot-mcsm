...
function botEvent() {
  bus.on("group_message_event", async (event) => {
    ...
    if (!data) {
      ...
      const serverName = inst.serverName;
      const ownerid = inst.ownerid;
      const groupuuid = inst.groupuuid;
      ...
      if (event.RawMessage === "开启 " + serverName && event.PeerId === groupuuid && event.SenderId === ownerid && use_regex === false) {
        ...
        if (event.RawMessage === "关闭 " + serverName && event.PeerId === groupuuid && event.SenderId === ownerid && use_regex === false) {
          await httpRequest_off(inst);
          event.Context.Reply(new MessageChain().Text("已执行关闭服务器: " + serverName));
          return;
        } else
          if (event.RawMessage === "重启 " + serverName && event.PeerId === groupuuid && event.SenderId === ownerid && use_regex === false) {
            await httpRequest_restart(inst);
            event.Context.Reply(new MessageChain().Text("已执行重启服务器: " + serverName));
            return;
          }else
          if (event.RawMessage === "强制关闭 " + serverName && event.PeerId === groupuuid && event.SenderId === ownerid && use_regex === false) {
    ...
    for (const inst of instances) {
      ...
      if (regexOn.test(event.RawMessage) && event.PeerId === groupuuid && event.SenderId === ownerid) {
      await httpRequest_on(inst);
      ...
      } else if (regexOff.test(event.RawMessage) && event.PeerId === groupuuid && event.SenderId === ownerid) {
      await httpRequest_off(inst);
      event.Context.Reply(new MessageChain().Text("已执行关闭服务器: " + serverName + " (正则匹配)"));
      logger.info(`[匹配分支] 强制关闭 | serverName: ${serverName} | 消息: ${event.RawMessage}`); // 新增
      } else if (regexRestart.test(event.RawMessage) && event.PeerId === groupuuid && event.SenderId === ownerid) {
      await httpRequest_restart(inst);
      event.Context.Reply(new MessageChain().Text("已执行重启服务器: " + serverName + " (正则匹配)"));
      logger.info(`[匹配分支] 强制关闭 | serverName: ${serverName} | 消息: ${event.RawMessage}`); // 新增
      } else if (regexKill.test(event.RawMessage) && event.PeerId === groupuuid && event.SenderId === ownerid) {
      await httpRequest_kill(inst);
